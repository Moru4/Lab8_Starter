...
describe('Basic user flow for Website', () => {
  ...
  it('Make sure <product-item> elements are populated', async () => {
    ...
    // check every other <product-item>
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    let sRoot = await prodItems[0].getProperty('shadowRoot');
    let button = await sRoot.$('button');
    await button.click();
    // let swapped = true;
    let iText = await button.getProperty('innerText');
    await button.click();
    // if (iText['_remoteObject'].value != 'Remove from Cart') {
    //   swapped = false;
    // }
    expect(iText['_remoteObject'].value).toBe('Remove from Cart');
  }, 2500);
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    let cartCount = await page.$('#cart-count');
    let cartCountText = await cartCount.getProperty('innerText');
    if (cartCountText['_remoteObject'].value != '20') {
      remembered = false;
    }
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    let cartCount = await page.$('#cart-count');
    let cartCountText = await cartCount.getProperty('innerText');
    if (cartCountText['_remoteObject'].value != '0') {
      remembered = false;
    }
    expect(remembered).toBe(true);
  }, 10000);
